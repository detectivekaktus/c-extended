...
typedef enum { C_DEBUG, C_INFO, C_WARNING, C_ERROR } Log_Level;
  ...
  do {                                                                         \
    if ((sb)->size + strlen(str) + 1 >= (sb)->capacity) {                      \
      while ((sb)->size + strlen(str) + 1 >= (sb)->capacity)                   \
        (sb)->capacity =                                                       \
            (sb)->capacity == 0 ? DA_INIT_CAPACITY : (sb)->capacity * 2;       \
...
#define cstrsplit(str, deli)                                                   \
  do {                                                                         \
    strtok(str, deli);                                                         \
    char *curtok;                                                              \
    while (curtok != NULL) {                                                   \
      curtok = strtok(NULL, deli);                                             \
    }                                                                          \
  } while (0)
